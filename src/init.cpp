...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: zaca.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: zacad.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizezacaamount=<n> " + _("Keep N Zaca anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Zaca is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Zaca is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Zaca version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Zaca server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Zaca") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Zaca to complete") << "\n";
    ...
    nAnonymizeZacaAmount = GetArg("-anonymizezacaamount", 0);
    if(nAnonymizeZacaAmount > 999999) nAnonymizeZacaAmount = 999999;
    if(nAnonymizeZacaAmount < 2) nAnonymizeZacaAmount = 2;
    ...
    LogPrintf("Anonymize Zaca Amount %d\n", nAnonymizeZacaAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ZACA+1000 == (.1ZACA+100)*10
       10ZACA+10000 == (1ZACA+1000)*10
    */
