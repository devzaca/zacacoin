...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeZacaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeZacaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeZacaAmount = strAnonymizeZacaAmount.remove(strAnonymizeZacaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZacaAmount + " ZACA / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeZacaAmount*COIN) nMaxToAnonymize = nAnonymizeZacaAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeZacaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeZacaAmount));
        strAnonymizeZacaAmount = strAnonymizeZacaAmount.remove(strAnonymizeZacaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZacaAmount + " ZACA / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
        ...
        strAmountAndRounds = "<span style='color:red;'>" +
                ...
                " ZACA / " + tr("%n Rounds", "", nDarksendRounds) + "</span>";
    }
    ui->labelAmountRounds->setText(strAmountAndRounds);
