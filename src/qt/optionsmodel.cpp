...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeZacaAmount"))
        settings.setValue("nAnonymizeZacaAmount", 1000);
    nAnonymizeZacaAmount = settings.value("nAnonymizeZacaAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeZacaAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeZacaAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeZacaAmount:
            return QVariant(nAnonymizeZacaAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeZacaAmount:
            nAnonymizeZacaAmount = value.toInt();
            settings.setValue("nAnonymizeZacaAmount", nAnonymizeZacaAmount);
            emit AnonymizeZacaAmountChanged(nAnonymizeZacaAmount);
            break;
        default:
            break;
        }
